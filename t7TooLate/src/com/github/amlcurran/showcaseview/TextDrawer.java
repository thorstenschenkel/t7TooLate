...
class TextDrawer {
	...
	public TextDrawer(final Resources resources, final Context context) {
	...
	public void draw(final Canvas canvas) {
		if (shouldDrawText()) {
			final float[] textPosition = getBestTextPosition();
			final int width = Math.max(0, (int) mBestTextPosition[INDEX_TEXT_WIDTH]);
			...
			if (!TextUtils.isEmpty(textString)) {
				...
				final float offsetForTitle = titleLayout != null ? titleLayout.getHeight() : 0;
				if (textLayout != null) {
					canvas.translate(textPosition[INDEX_TEXT_START_X], textPosition[INDEX_TEXT_START_Y]
							+ offsetForTitle);
	...
	public void setContentText(final CharSequence details) {
		if (details != null) {
			final SpannableString ssbDetail = new SpannableString(details);
	...
	public void setContentTitle(final CharSequence title) {
		if (title != null) {
			final SpannableString ssbTitle = new SpannableString(title);
	 ...
	 * @param showcase
	 */
	public void calculateTextPosition(final int canvasW, final int canvasH, final boolean shouldCentreText,
			final Rect showcase) {

		final int[] areas = new int[4]; // left, top, right, bottom
	...
	public void setTitleStyling(final int styleId) {
		titleSpan = new TextAppearanceSpan(this.context, styleId);
		setContentTitle(titleString);
	}

	public void setDetailStyling(final int styleId) {
	...
	public void setContentPaint(final TextPaint contentPaint) {
	...
	public void setTitlePaint(final TextPaint textPaint) {
	...
	public void setTitleTextAlignment(final Layout.Alignment titleTextAlignment) {
		this.titleAlignment = titleTextAlignment;
	}

	public void forceTextPosition(@ShowcaseView.TextPosition final int textPosition) {
